...
int main() {
  ...
  char c;
  cin >> t;
  // we precompute some shit:
  ll mod = 1e9 + 7;
  v64 facs((int)(2 * 1e5 + 1));
  v64 ifacs(2 * 1e5 + 1);
  ifacs[0] = 1;
  ifacs[1] = 1;
  forsn(i, 2, 2 * 1e5 + 1) {
    ll inv_i = modpow(i, mod - 2, mod);
    ifacs[i] = (ifacs[i - 1] * inv_i) % mod;
  }
  facs[0] = 1;
  facs[1] = 1;
  forsn(i, 2, 2 * 1e5 + 1) { facs[i] = (i * facs[i - 1] % mod) % mod; }
  forn(i, t) {
    cin >> u >> v >> n;
    ll d = v - u + 1;
    if (d == 1) {
      ...
      ll numer = facs[n + d - 3];
      ll ans = (numer * ifacs[d - 1] % mod) * ifacs[n - 2] % mod;
      cout << ans << ln;
    }
  }
  return 0;
