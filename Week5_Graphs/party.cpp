...
pair<ll, ll> treeDFS(vv64 &adj, ll node, ll parent, v64 &fun) {
  ll no_invite = 0;
  ll invite = fun[node];
  // go through children of node
  for (auto i : adj[node]) {
    auto [c_no_invite, c_invite] = treeDFS(adj, i, node, fun);
    no_invite += max(c_no_invite, c_invite);
    invite += c_no_invite;
  }

  return {no_invite, invite};
}
...
int main() {
  ...
  pair<ll, ll> total = (treeDFS(adj, 1, 0, fun));
  ll maximum = max(total.fi, total.se);
  cout << maximum;
  return 0;
}